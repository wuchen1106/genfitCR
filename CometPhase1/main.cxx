...
TTree* make_branch(const char* tname, struct tree_value* tv)
{
	...
	t->Branch("tid",&tid,"tid/I");
	t->Branch("vid",vid,"vid[124]/C");
	t->Branch("prid",prid,"prid[124]/C");
...
int read_hit(int iev, char* root_file, TVector3& posini, TVector3& momini)
	// FIXME: nned to change to drift_r
{
	if (root_opened==0) {
		if (root_file==NULL)
			exit(1);
		...
		root_t->SetBranchAddress("evt_num",&root_evt_num);
		root_t->SetBranchAddress("run_num",&root_run_num);
		root_t->SetBranchAddress("pid",&root_pid);
		root_t->SetBranchAddress("tid",&root_tid);
		root_t->SetBranchAddress("vid",root_vid);
		root_t->SetBranchAddress("prid",root_prid);
		root_t->SetBranchAddress("ini_x",&root_ini_posx);
		root_t->SetBranchAddress("ini_y",&root_ini_posy);
		root_t->SetBranchAddress("ini_z",&root_ini_posz);
		root_t->SetBranchAddress("ini_px",&root_ini_momx);
		root_t->SetBranchAddress("ini_py",&root_ini_momy);
		root_t->SetBranchAddress("ini_pz",&root_ini_momz);
		root_t->SetBranchAddress("nhits",&root_nhits);
		root_t->SetBranchAddress("hits_x",root_hits_posx);
		root_t->SetBranchAddress("hits_y",root_hits_posy);
		root_t->SetBranchAddress("hits_z",root_hits_posz);
		root_t->SetBranchAddress("hits_px",root_hits_momx);
		root_t->SetBranchAddress("hits_py",root_hits_momy);
		root_t->SetBranchAddress("hits_pz",root_hits_momz);
		root_opened = 1;
	}
	...
	evt_num = root_evt_num;
	run_num = root_run_num;
	pid = root_pid;
	tid = root_tid;
	strcpy(vid, root_vid);
	strcpy(prid, root_prid);
...
int main(int argc, char** argv)
{
	...
	for (int iev=0; iev<arg_total; iev++) {
		...
		for (int i=0; i<nhits; i++) {
			...
			// get wire info
			int ilayer = g_hits_det.ilayer[i];
			int icell  = g_hits_det.icell[i];
			...
			// get the hit point
            double rdrift_smeared = g_hits_det.rdrift_smeared[i];
			...
			fitTrack.addHit(
					...
					new WireHit(point1,point2,rdrift_smeared,g_rdrift_err,false),
		...
		// FIXME should do fitting in another way
